        ...
        if (documentoExistente) {
            ...
            if (datosRanking.totalScore > documentoExistente.totalScore) {
                ...
                const resultadoActualizacion = await collection.updateOne(
                    ...
                    { $set: { totalScore: datosRanking.totalScore, nom: datosRanking.nom } }
                );
            ...
            // Si no existe, insertar un nuevo documento con los datos
            const resultadoInsercion = await collection.insertOne(datosRanking);
            console.log(`Nuevo registro insertado para el DNI: ${datosRanking.dni} con totalScore: ${datosRanking.totalScore}`);
        }
    } catch (error) {
